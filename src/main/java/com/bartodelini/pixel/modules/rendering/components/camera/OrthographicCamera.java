...
/**
 * An <i>OrthographicCamera</i> is a {@linkplain Camera} with an orthographic-view frustum.
 *
 * @author Bartolini
 * @version 1.0
 */
public class OrthographicCamera extends Camera {
    ...
    /**
     * Allocates a new {@code OrthographicCamera} by passing in the {@code fov}, {@code aspectRatio}, values for the
     ...
     */
    public OrthographicCamera(float fov, float aspectRatio, float near, float far, int backgroundColor, PostProcessingFilter... filters) {
        super(fov, aspectRatio, near, far, backgroundColor, filters);
    }

    @Override
    public Matrix4f calculateProjectionMatrix() {
        return Matrices.getOrthographicProjection(getFov(), getAspectRatio(), getNear(), getFov());
    }
}